...
flowchart TB
    ...
    subgraph 绘制层
        E[DrawingCanvasView\n触摸处理/绘制/缩放/Undo/Redo]
        F[DrawingToolbar\n颜色/粗细/画笔/橡皮擦]
    end

    subgraph 数据与存储层
        G[StrokeData / DrawingData\n笔迹模型]
        H[DrawingSerializer\nJSON 序列化 + PNG 缩略图]
...
### 3.0 Mermaid：进入画板与保存回填时序图

```mermaid
sequenceDiagram
    participant U as 用户
    participant E as NoteEditScreen
    participant M as MainActivity
    participant S as DrawingScreen
    participant V as DrawingCanvasView
    participant DS as DrawingSerializer
    participant N as Note
    participant DB as Room

    U->>E: 点击“添加手绘 / 编辑手绘”
    E->>M: 切换到 DrawingScreen
    M->>S: 创建页面并传入 existingDrawingPath
    S->>V: 创建 AndroidView + DrawingCanvasView
    alt 存在旧手稿
        S->>DS: loadFromFile(existingDrawingPath)
        DS-->>S: DrawingData
        S->>V: loadDrawingData(data)
    end

    U->>V: 单指绘制 / 双指缩放 / 橡皮擦
    V->>V: 更新 currentPath / commitBitmap / strokeHistory

    U->>S: 点击保存
    S->>V: getDrawingData()
    V-->>S: DrawingData
    S->>DS: saveToFile(context, id, data)
    DS-->>S: drawingPath
    S->>V: exportBitmap()
    V-->>S: Bitmap
    S->>DS: saveThumbnail(context, id, bitmap)
    DS-->>S: thumbnailPath
    S-->>M: 回传 drawingPath + thumbnailPath
    M->>E: 回填编辑页状态
    E->>N: 写入 drawingPath / drawingThumbnailPath
    N->>DB: 保存 Note
```
...
flowchart LR
    A[触摸输入] --> B{当前手势类型}
    B -->|单指绘制| C[currentPath\ncurrentPoints]
    B -->|双指操作| D[scale / panX / panY]
    C --> E[finishStroke]
    E --> F[strokeHistory]
    E --> G[commitCanvas]
    G --> H[commitBitmap\n历史已提交图层]
